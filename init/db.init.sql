...
begin
    ...
    -- update ID values to node IDs
    execute format(E'alter table %I add column if not exists node_id integer;', _tbl);
    ...
    execute format(E'update %I set node_id = refs.id from (
         where nodes.type=\'%s\' order by node_id) as refs', _tbl, _tbl);
...
DROP SEQUENCE IF EXISTS mlp_captures_id_seq;
CREATE SEQUENCE mlp_captures_id_seq INCREMENT 1 MINVALUE 1 MAXVALUE 9223372036854775807 START 1 CACHE 1;
...
CREATE INDEX "index_mlp_captures_on_owner_id" ON "public"."captures" USING btree ("owner_id");
...
DROP SEQUENCE IF EXISTS historic_captures_id_seq;
CREATE SEQUENCE historic_captures_id_seq INCREMENT 1 MINVALUE 1 MAXVALUE 9223372036854775807 START 1 CACHE 1;
...
CREATE TABLE "public"."historic_captures" (
      ...
      "published" boolean,
      CONSTRAINT "historic_captures_pkey" PRIMARY KEY ("id")
) WITH (oids = false);

-- Historic Captures

DROP TABLE IF EXISTS "mlp_modern_captures";
DROP SEQUENCE IF EXISTS mlp_modern_captures_id_seq;
CREATE SEQUENCE mlp_modern_captures_id_seq INCREMENT 1 MINVALUE 1 MAXVALUE 9223372036854775807 START 1 CACHE 1;
...
CREATE TABLE "public"."mlp_modern_captures" (
      ...
      "capture_owner_type" character varying(255),
      "published" boolean,
      CONSTRAINT "mlp_modern_captures_pkey" PRIMARY KEY ("id")
) WITH (oids = false);
...
DROP SEQUENCE IF EXISTS mlp_images_id_seq;
CREATE SEQUENCE mlp_images_id_seq INCREMENT 1 MINVALUE 1 MAXVALUE 9223372036854775807 START 1 CACHE 1;
...
CREATE TABLE "public"."mlp_capture_images" (
       "image_id" integer NOT NULL,
       "image_type" image_types,
       "owner_id" integer NOT NULL,
        CHECK (image_type IN ('modern_capture', 'historic_capture')),
...
CREATE INDEX "index_capture_images_on_owner_id" ON "public"."mlp_capture_images" USING btree ("owner_id");
